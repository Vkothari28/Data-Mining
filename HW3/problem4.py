...
def remove_sink_nodes(A):
    #########################################
    ## INSERT YOUR CODE HERE (5 points)
    for elem in range(len(np.all(A == 0, axis=0))):
        if np.all(A == 0, axis=0)[elem]:
            A[:, elem] = 1
...
def compute_G(S, alpha):
    ...
    G = alpha * S + (1 - alpha) * np.ones_like(S) / len(S)
    #########################################
    return G
    # -----------------
    '''  
