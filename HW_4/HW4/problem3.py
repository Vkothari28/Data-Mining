...
def train_user_j(X, y, a=1e-05):
    #########################################
    ## INSERT YOUR CODE HERE (7 points)
    Uj = np.linalg.inv(X.T @ X + a * np.identity(np.shape(X)[1])) @ (X.T @ y)
    #########################################
    return Uj
    # -----------------
    '''  
...
def update_U(R, I, a=1e-05):
   ...
   ## y, p = extract_user_j(R[:,0], I)


  ###  j, k = extract_user_j(R[:,1], I)
   ## f = train_user_j(y, p, a)


    ##v = train_user_j(j, k, a)

   ## U=[f,v]
    count=0
    for i in range (R.shape[1]):
        y, p = extract_user_j(R[:, i], I)
        f =[train_user_j(y,p,a)]
        if(count ==0):
            U=f
            print(U)
...
def collaborative_filtering(R, k=5, a=1e-05, n_steps=20):
    ...
    for i in range(n_steps):  # repeat n_steps
        #########################################
